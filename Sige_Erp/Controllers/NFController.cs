...
namespace Sige_Erp.Controllers
{
    public class NFController : Controller
    {
        ...
        public IActionResult Index(NFModel nota)
        {
            nota.AbaAtiva = false;
            nota.ListaNotas = new List<NFModel>();
        ...
        public bool ValidacaoCadastro(NFModel nota)
        {
            ...
            {
                throw new Exception($"Por favor, informe a ChaveNf.");
            }
        ...
        [HttpPost]
        public IActionResult CadastroNFManutencao(NFModel nota)
        {
            try
            {
                if (nota.NrSeqNf <= 0)
        ...
        [HttpPost]
        public IActionResult AcaoDesejadaNF(string acao, string IdsSelecionados, NFModel nota)
        {

            nota.ListaNotas ??= new List<NFModel>();
            List<NFModel> data = new List<NFModel>();
            switch (acao)
            {
                ...
                case "Excluir":
                        ...
                        foreach (int idSelecionado in nota.IdsSelecionados)
                        {
                            using (DAL objDAL = new DAL())
                            {
                                ...
                                try
                                {
                                    ...
                                    nota.NrSeqNf = idSelecionado;
                                    if (nota.NrSeqNf > 0) nota.Excluir(objDAL);
                    ...
                    break;
                case "Imprimir":
                    List<NFModel> lista = new List<NFModel>();
                        ...
                        try
                        {
                            ...
                            else
                            {
                                try
                                {
                                    ...
                                    // Dados do cliente
                                    PdfPCell clienteCell = new PdfPCell(new Phrase($"Cliente: {lista[0].NomeDoFornecedor}\nEndereço: {lista[0].EnderecoFornecedor}\nCNPJ: {lista[0].CnpjFornecedor}", fontePadrao));
                                    clienteCell.Border = PdfPCell.NO_BORDER;
                                    tableInfo.AddCell(clienteCell);

                                    // Dados da nota fiscal
                                    PdfPCell nfCell = new PdfPCell(new Phrase($"Número da NF: {lista[0].NrNf}\nSérie: {lista[0].Serie}\nData de Emissão: {lista[0].DataCadastro:dd/MM/yyyy}", fontePadrao));
                                    ...
                                    foreach (var header in new[] { "Nr Nf", "Nr Pedido", "Fornecedor", "Produto", "Quantidade", "Valor Total", "Imposto", "Frete", "Data Cadastro" })
                                    {
                                    ...
                                    {
                                        foreach (var property in new[] { item.NrNf, item.NrSeqPedido.ToString(), item.NomeDoFornecedor, item.NomeDoProduto, item.Quantidade.ToString(), item.ValorTotal.ToString(), item.ValorImpostos.ToString(), item.ValorFrete.ToString(), item.DataCadastro.ToString("dd/MM/yyyy") })
                                        {
        ...
        public JsonResult BuscarFornecedores(string termo)
        {
            ...
            var fornecedores = fornecedorModel.Pesquisar(fornecedorModel)
                                              .Select(fornecedor => new { value = fornecedor.NomeDoFornecedor, label = fornecedor.NomeDoFornecedor, nrSeqPessoa = fornecedor.NrSeqPessoa, nrSeqFornecedor = fornecedor.NrSeqFornecedor });
